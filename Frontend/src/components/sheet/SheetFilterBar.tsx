...
export const SheetFilterBar = ({
  ...
  return (
    <div className="mb-6 space-y-3 bg-white dark:bg-slate-900/80 p-4 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-sm">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-10 py-2 text-sm bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all"
          />
          {searchQuery && (
            <button
              type="button"
        ...
        {availablePlatforms.length > 0 && (
          <div className="shrink-0">
            <select
              ...
              className="w-full sm:w-auto px-3 py-2 text-xs font-medium rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
      ...
      {/* Bottom row: Filter chips (Difficulty & Solved state) */}
      <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-100 dark:border-slate-800/80">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* Solved / Unsolved filter chips */}
          <div className="inline-flex rounded-lg p-0.5 bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs">
            {(["ALL", "SOLVED", "UNSOLVED"] as SolvedFilter[]).map((st) => {
              const isActive = solvedFilter === st;
              const label =
                st === "ALL" ? "All Status" : st === "SOLVED" ? "Solved" : "Unsolved";
              return (
                <button
                  key={st}
                  type="button"
                  onClick={() => setSolvedFilter(st)}
                  className={`px-2.5 py-1 rounded-md font-medium transition-all ${
                    isActive
